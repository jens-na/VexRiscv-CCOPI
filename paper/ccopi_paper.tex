...
``ex1'' is the name of the instruction function in the detailed example
above. Each signal, which belongs to this specific instruction function
starts with ``\code{ex1\_}''.

\section{Parameters}
Another advantage of CCOPI is the small overhead its generates in matter of 
hardware size and communication latency. Since the size of a result computed 
by an coprocessor is known before the actual hardware is generated, the result 
can be transmitted within one clock cycle via the Ready/Valid Interface.

The required number of LUTs and flip-flops for VexRiscv depends on the Plugins 
it uses. For example extensions for pipeline hazard managment or debugging can 
be added. For test purposes a minimal configuration was chosen, which needed a 
total of 500 LUTs and 592 flip-flops on an Artix-7 FPGA. After adding an trivial 
Custom Coprocessor, which solely fills the destination registers with zeros, to 
VexRiscv only an additional amount of 34 LUTs and 2 flip-flops were needed. This
is a rather low price for the huge amount of flexibility which CCOPI
offers.
\section{Future Work}
